<x-layouts.admin title="Tambah Role">
    <div class="px-4 py-6 sm:px-6 lg:px-8">
        <div class="mb-6">
            <h1 class="text-xl font-semibold text-slate-900">Tambah Role</h1>
            <p class="mt-1 text-sm text-slate-600">Buat role baru dan atur permission yang dimiliki.</p>
        </div>

        <form action="{{ route('admin.roles.store') }}" method="POST" class="bg-white shadow-sm ring-1 ring-slate-200 sm:rounded-xl">
            @csrf
            ...
            <div class="px-4 py-6 sm:p-8">
                <div class="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                    <div class="sm:col-span-4">
                        ...
                        <div class="mt-2">
                            <input type="text" name="name" id="name" required class="block w-full rounded-md border-0 py-1.5 text-slate-900 shadow-sm ring-1 ring-inset ring-slate-300 placeholder:text-slate-400 focus:ring-2 focus:ring-inset focus:ring-blue-600 sm:text-sm sm:leading-6">
                        </div>
                    </div>
                                                ...
                                                @forelse($category->permissions as $permission)
                                                    <div class="flex items-start permission-item" data-search="{{ strtolower(($permission->label ?? $permission->name) . ' ' . $permission->name) }}">
                                                        <div class="flex h-6 items-center">
                                                            <input id="perm_{{ $permission->id }}" name="permissions[]" value="{{ $permission->name }}" type="checkbox" class="perm-checkbox h-4 w-4 rounded border-slate-300 text-orange-500 focus:ring-orange-500">
                                                        </div>
            ...
            <div class="flex items-center justify-end gap-x-6 border-t border-slate-200 px-4 py-4 sm:px-8 bg-slate-50 rounded-b-xl">
                ...
                <button type="submit" class="rounded-md bg-blue-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-500">Simpan Role</button>
            </div>
        </form>
    </div>
</x-layouts.admin>
