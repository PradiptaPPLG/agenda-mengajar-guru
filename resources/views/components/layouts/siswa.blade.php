<x-layouts.app>
    <x-slot:title>{{ $title ?? 'Siswa' }}</x-slot:title>
        ...
        <nav class="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-slate-200">
            <div class="flex items-center justify-around px-2 h-16" style="padding-bottom: env(safe-area-inset-bottom)">
                <a href="{{ route('siswa.dashboard') }}"
                   class="flex flex-col items-center gap-0.5 px-4 py-2 rounded-xl transition-colors {{ request()->routeIs('siswa.dashboard') ? 'text-emerald-600' : 'text-slate-500 hover:text-slate-900' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('profile.index') }}"
                   class="flex flex-col items-center gap-0.5 px-4 py-2 rounded-xl transition-colors {{ request()->routeIs('profile.*') ? 'text-emerald-600' : 'text-slate-500 hover:text-slate-900' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
