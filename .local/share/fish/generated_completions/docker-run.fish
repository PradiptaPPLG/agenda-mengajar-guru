...
complete -c docker-run -l add-host -d '   Add a custom host-to-IP mapping (host=ip, or host:ip) Add a line to /etc/h…'
complete -c docker-run -l annotation -d '   Add an annotation to the container (passed through to the OCI runtime)'
...
complete -c docker-run -s c -l cpu-shares -d '   CPU shares (relative weight) By default, all containers get the same propo…'
...
complete -c docker-run -l cpu-percent -d '   Limit the percentage of CPU available for execution by a container running…'
...
complete -c docker-run -s d -l detach -d '   Detached mode: run the container in the background and print the new    co…'
complete -c docker-run -l detach-keys -d '   Override the key sequence for detaching a container; key is a single    ch…'
...
complete -c docker-run -l memory-swap -d '   Combined memory plus swap limit; S is an optional suffix which can be    o…'
...
complete -c docker-run -l rm -d '   Automatically remove the container and its associated anonymous volumes wh…'
...
complete -c docker-run -l stop-signal -d '   Signal to stop the container'
