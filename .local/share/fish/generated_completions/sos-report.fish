...
complete -c sos-report -s k -l plugin-option -d '--plugopts=PLUGNAME. PLUGOPT[=VALUE] Specify plug-in options'
...
complete -c sos-report -l encrypt-key -d 'Encrypts the resulting archive that sos report produces using GPG'
...
complete -c sos-report -l threads -d 'Specify the number of threads sos report will use for concurrency'
...
complete -c sos-report -l plugopts -d 'Specify plug-in options'
