...
complete -c docker-create -l cgroupns -d '	Cgroup namespace to use (host|private) \'host\':    Run the container in the D…'
...
complete -c docker-create -l rm -d '	Automatically remove the container and its associated anonymous volumes when…'
...
complete -c docker-create -l use-api-socket -d '	Bind mount Docker API socket and required auth'
...
complete -c docker-create -s w -l workdir -d '	Working directory inside the container SEE ALSO docker(1)'
