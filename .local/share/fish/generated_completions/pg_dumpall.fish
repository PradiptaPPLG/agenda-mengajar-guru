...
.br
--clean' -d 'Emit SQL commands to DROP all the dumped databases, roles, and tablespaces be…'
...
complete -c pg_dumpall -l if-exists -d 'Use DROP .  IF EXISTS commands to drop objects in --clean mode'
...
complete -c pg_dumpall -l restrict-key -d 'Use the provided string as the psql restrict key in the dump output'
...
complete -c pg_dumpall -s X -d '(--no-psqlrc) option when restoring a database from a pg_dumpall script to en…'
